...
/**
 * 月次帳票出力Entity用スーパークラス.
 * 
 * @author yamaguchi
 * @version 20080408
 */
public abstract class LACSMReportEntityBase extends EntityBase {

	/**
	 * 対象期間開始.
	 */
	protected String			taisyouFrom		= "";

	/**
	 * 対象期間終了.
	 */
	protected String			taisyouTo		= "";

	/**
	 * 月次帳票-契約額300万円以下除外.
	 */
	protected String			gtjkeiyakuGaku	= "0";

	/**
	 * 月次帳票-リース期間1年未満除外.
	 */
	protected String			gtjleaseKikan	= "0";

	/**
	 * 月次帳票-再リース契約除外.
	 */
	protected String			gtjsaiLease		= "0";

	/**
	 * 月次帳票-中途解約契約除外.
	 */
	protected String			gtjtyutoKaiyaku	= "0";
	 ...
	 *            帳票出力Bean
	 */
	public LACSMReportEntityBase(DBModelBase piModel, LACSCommonBean piCommonBean, LACSMReportBean piReportBean) {
		super(piModel);

		this.keiTermFrom = piReportBean.getTermFrom().getYYYYMMDD();
		this.dateFrom = piReportBean.getTermFrom().getYYYYMMDD();
		this.termNum = piReportBean.getTermNum();

		this.keiTermTo = piReportBean.getTermTo().getYYYYMMDD();

		this.taisyouFrom = this.keiTermFrom;
		this.taisyouTo = this.keiTermTo;

		this.gtjkeiyakuGaku = piReportBean.getGtjKeiyakuGaku();
		this.gtjleaseKikan = piReportBean.getGtjLeaseKikan();
		this.gtjsaiLease = piReportBean.getGtjSaiLease();
		this.gtjtyutoKaiyaku = piReportBean.getGtjTyutoKaiyaku();
		this.leasCompanyCode = piReportBean.getLeasCompany().getValue();
				
     	this.keiyakuNo = piReportBean.getKeiyakuNo();						
		//this.keiyakuNo = piReportBean.getHyoujiKeiyakuNo();
		this.bukkenNo = piReportBean.getBukkenNo();
		this.bukkenEdaNo = piReportBean.getBukkenEdaNo();
	...
	protected String getCoreSQL() {
		...
		buf.append("      ,GNKSK_KEIJ_YM_ZANKA_LAST " + "\n");
		buf.append("      ,FKN_KEIJ_YM_MAX_KAI " + "\n");
		...
		buf.append("      ,KEI.STAX_IKT_KOJ_KBN " + "\n");
		buf.append("					,(SELECT MAX(MAX_KAI_REC0.KEIJ_YM) FROM T_UKB_GNKSK MAX_KAI_REC0 WHERE MAX_KAI_REC0.LC_CD = BKN.LC_CD " + "\n");
		buf.append("						AND MAX_KAI_REC0.KEI_NO = BKN.KEI_NO " + "\n");
		buf.append("						AND MAX_KAI_REC0.BKN_NO = BKN.BKN_NO " + "\n");
		buf.append("						AND MAX_KAI_REC0.BKN_EDANO = BKN.BKN_EDANO " + "\n");
		buf.append("						AND MAX_KAI_REC0.KEIJ_HOHO_KBN = BKN.SKK_KEIJ_HOHO_KBN " + "\n");
		buf.append("						AND MAX_KAI_REC0.KAI_REC_FLG ='0' " + "\n");
		buf.append("						AND MAX_KAI_REC0.KEIJ_YM BETWEEN START_KEIJ_YM AND END_KEIJ_YM) GNKSK_KEIJ_YM_MAX_KAI_REC " + "\n");
		buf.append("					,(SELECT MIN(MAX_KAI_REC0.KEIJ_YM) FROM T_UKB_GNKSK MAX_KAI_REC0 WHERE MAX_KAI_REC0.LC_CD = BKN.LC_CD " + "\n");
		buf.append("						AND MAX_KAI_REC0.KEI_NO = BKN.KEI_NO " + "\n");
		buf.append("						AND MAX_KAI_REC0.BKN_NO = BKN.BKN_NO " + "\n");
		buf.append("						AND MAX_KAI_REC0.BKN_EDANO = BKN.BKN_EDANO " + "\n");
		buf.append("						AND MAX_KAI_REC0.KEIJ_HOHO_KBN = BKN.SKK_KEIJ_HOHO_KBN " + "\n");
		buf.append("						AND MAX_KAI_REC0.KAI_REC_FLG ='0' " + "\n");
		buf.append("						AND MAX_KAI_REC0.KEIJ_YM BETWEEN START_KEIJ_YM AND END_KEIJ_YM) GNKSK_KEIJ_YM_MIN " + "\n");
		...
		buf.append("                    AND    FKN_HEAD.BKN_NO = B.BKN_NO " + "\n");
		buf.append("                    AND    FKN_HEAD.BKN_EDANO = B.BKN_EDANO " + "\n");
		buf.append("                    AND    FKN_HEAD.KAI_REC_FLG = '0') FKN_KEIJ_YM_MAX_KAI " + "\n");
		buf.append("                  ,(SELECT MAX(KEIJ_YM) " + "\n");
